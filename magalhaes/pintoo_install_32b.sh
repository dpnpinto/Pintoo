#!/bin/bash
# Instalação do Pintoo 32-bit (Gentoo a la Pinto)
...
echo "=== 5. Configurar o Portage make.conf (Binary Packages 32-bit) ==="
cat << 'EOF' > /mnt/gentoo/etc/portage/make.conf
# These settings were set by the catalyst build script that automatically
# built this stage.
# Please consult /usr/share/portage/config/make.conf.example for a more
# detailed example.
COMMON_FLAGS="-O2 -pipe -march=i686" # Architecture to use (32-bit)
...
echo "sys-kernel/gentoo-kernel-bin ~x86" > /etc/portage/package.accept_keywords/kernel
echo "virtual/dist-kernel ~x86" >> /etc/portage/package.accept_keywords/kernel
